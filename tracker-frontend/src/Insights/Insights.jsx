...
/**
 * transactions: array of { amount: number|string, date: string (ISO), category: string, type: string }
 */
  ...
  const {
    ...
    transactions.forEach((t) => {
      ...
      if (d >= startThisYear && d <= now) {
        ...
        if (amt > maxThis.amount)
          maxThis = { amount: amt, date: d, category: t.category };
      }
      // YTD last year
      if (d >= startLastYear && d <= endLastYearYTD) {
        ...
        if (amt > maxLastYTD.amount)
          maxLastYTD = { amount: amt, date: d, category: t.category };
      }
      // Full last year
      if (d >= startLastYear && d <= endLastYearFull) {
        ...
        if (amt > maxLastFull.amount)
          maxLastFull = { amount: amt, date: d, category: t.category };
      }
    });

    const pctChange =
      sumLastYTD > 0 ? ((sumThis - sumLastYTD) / sumLastYTD) * 100 : null;
  ...
  return (
    <>
        ...
        <Card.Body>
          <Row>
            <Col>
              <h5>{thisYear}</h5>
              <p>
                <strong>Spending:</strong> ${ytdThisYear.toFixed(2)}
              </p>
              <p>
                <strong>Avg Monthly:</strong> ${avgMonthlyThisYear.toFixed(2)}
              </p>
              <p>
                <strong>Top Category:</strong> {topCategoryThisYear || "N/A"}{" "}
                {topCategoryThisYear
                  ? `($${sumCatYear(topCategoryThisYear, thisYear)})`
                  : ""}
              </p>
              <p>
                <strong>Largest Txn:</strong> $
                {largestTxnThisYear.amount.toFixed(2)} on{" "}
                {fmt(largestTxnThisYear.date)}
              </p>
            </Col>
            <Col>
              <h5>{lastYear}</h5>
              <p>
                <strong>Spending:</strong> ${ytdLastYear.toFixed(2)}
              </p>
              <p>
                <strong>Avg Monthly:</strong> ${avgMonthlyLastYTD.toFixed(2)}
              </p>
              <p>
                <strong>Top Category:</strong> {topCategoryLastYTD || "N/A"}{" "}
                {topCategoryLastYTD
                  ? `($${sumCatYear(topCategoryLastYTD, lastYear)})`
                  : ""}
              </p>
              <p>
                <strong>Largest Txn:</strong> $
                {largestTxnLastYTD.amount.toFixed(2)} on{" "}
                {fmt(largestTxnLastYTD.date)}
              </p>
            </Col>
          </Row>
          {pctChangeYTD !== null && (
            <p className="mt-3">
              <strong>YoY Change:</strong> {pctChangeYTD.toFixed(1)}%
            </p>
          )}
        </Card.Body>
      </Card>
      ...
      <Card border="success" className="mb-3 ">
        ...
        <Card.Body>
          <p>
            <strong>Total Spending:</strong> ${fullLastYear.toFixed(2)}
          </p>
          <p>
            <strong>Avg Monthly:</strong> ${avgMonthlyFullLast.toFixed(2)}
          </p>
          <p>
            <strong>Top Category:</strong> {topCategoryFullLast || "N/A"}{" "}
            {topCategoryFullLast
              ? `($${sumCatYear(topCategoryFullLast, lastYear)})`
              : ""}
          </p>
          <p>
            <strong>Largest Txn:</strong> $
            {largestTxnFullLast.amount.toFixed(2)} on{" "}
            {fmt(largestTxnFullLast.date)}
          </p>
        </Card.Body>
      </Card>
    </>
